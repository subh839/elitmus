...
export default function Home() {
    ...
    const calculateScore = () => {
      ...
      var x=(numCorrect / denominator)*100;
      console.log(x);
      if(denominator) updateScore(x);
      return denominator && x;
    }

    const updateScore = async (score) => {
      try{
        const {data} = await axios.patch(`https://elitmus-backend-9rw7.onrender.com/api/users/${userId}`, { score });
      } catch(err) {
        console.error(err);
      }
    }

    const storeX = async (x) => {
      try {
        const response = await axios.post('https://elitmus-backend-9rw7.onrender.com/api/users', { x });
...
<button className="btn btn-sm fw-bold btn-outline-secondary btn-light me-2"   style={{borderColor: "navy"}} onClick={handleReload}>Reset</button>
        </header>
